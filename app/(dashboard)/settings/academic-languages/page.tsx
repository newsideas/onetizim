...
export default async function AcademicLanguagesPage() {
  const config = getReference("academic-languages");
  ...
  return (
    <ListPageShell title={config.title} subtitle={config.subtitle}>
      <ReferenceManager refKey="academic-languages" config={config} rows={data ?? []} />
    </ListPageShell>
  );
}
