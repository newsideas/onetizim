...
export default async function ContractDiscountsPage() {
  const config = getReference("contract-discounts");
  ...
  return (
    <ListPageShell title={config.title} subtitle={config.subtitle}>
      <ReferenceManager refKey="contract-discounts" config={config} rows={data ?? []} />
    </ListPageShell>
  );
}
