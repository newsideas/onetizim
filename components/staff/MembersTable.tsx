...
export function MembersTable({ members, currentUserId }: { members: OrgMember[]; currentUserId: string }) {
  ...
  function changeRole(member: OrgMember, role: Role) {
    if (role === member.role) return;
    run(() => updateMemberRole(member.userId, role));
  }

  function remove(member: OrgMember) {
    if (!confirm(`"${member.name}"ning tizimga kirish huquqini bekor qilmoqchimisiz?`)) return;
